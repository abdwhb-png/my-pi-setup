...
export type ProfileOverridesMap = Partial<
    Record<AuditProfileName, ProfileOverrides>
>;
...
function isValidProfileName(value: unknown): value is AuditProfileName {
    return (
        typeof value === "string" &&
        (PROFILE_NAMES as readonly string[]).includes(value)
    );
}
...
export function normalizeAuditSettings(input: unknown): AuditSettings {
    ...
    if (isRecord(input.profiles)) {
        ...
        for (const name of PROFILE_NAMES) {
            ...
            if (isRecord(override)) {
                const valid: ProfileOverrides = {};
                for (const key of STANDARD_PROFILE_KEYS) {
