export interface RoleTransitionRole {
    name: string;
    handoffGuard?: string;
}
...
export interface RoleTransitionPolicyInput {
    from: RoleTransitionRole | null;
    to: RoleTransitionRole;
...
export async function evaluateRoleTransitionPolicies(
    ...
    const policies = Array.from(getRegistry().policies.values());
    for (const policy of policies) {
        // Policies are ordered and the first denial wins, so evaluation is intentionally sequential.
...
export async function authorizeRoleTransition(
    ...
    if (
        input.transition.kind === "startup" ||
        input.transition.kind === "restore"
    ) {
        return { allow: true };
    }
