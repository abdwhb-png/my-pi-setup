...
function loadLayersFromSettings<T>(
  ...
  try {
    ...
    const globalRaw = (manager.getGlobalSettings() as Record<string, unknown>)[settingsKey];
    const projectRaw = (manager.getProjectSettings() as Record<string, unknown>)[settingsKey];
 ...
 *      a. Global settings.json key → Project settings.json key
 *      b. If settings produced empty result: Global legacy file → Project legacy file
 *
 * Within a single source, if settingsKey produces data, legacyFilename is skipped
 * (cascade — not cumulative). Across multiple sources, results merge sequentially.
 */
