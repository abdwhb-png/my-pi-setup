...
describe("package-install-finalizer", () => {
  it("replaces a stale shim with a real symlink", async () => {
    ...
    writeFileSync(join(shimDir, "package.json"), JSON.stringify({ name: "pi-roles", type: "module" }));
    writeFileSync(join(shimDir, "index.js"), 'export * from "/old/path/index.js";\n');
    writeFileSync(join(shimDir, "protocol.js"), 'export * from "/old/path/protocol.js";\n');
    ...
    expect(lstatSync(join(agentDir, "node_modules", "pi-roles")).isSymbolicLink()).toBe(true);
    expect(realpathSync(join(agentDir, "node_modules", "pi-roles"))).toBe(realpathSync(packageRoot));
  });
  ...
  it("uses cached state to skip unchanged packages on subsequent runs", async () => {
    ...
    const first = packageFinalizer.repairConfiguredPiPackages({ cwd, agentDir, logger: { info() {}, warn() {} } });
    const second = packageFinalizer.repairConfiguredPiPackages({ cwd, agentDir, logger: { info() {}, warn() {} } });
  ...
  it("creates scoped package links by ensuring parent directories exist", () => {
    ...
    expect(changed).toContain(join(agentDir, "node_modules", "@juicesharp", "rpiv-ask-user-question"));
    expect(lstatSync(join(agentDir, "node_modules", "@juicesharp", "rpiv-ask-user-question")).isSymbolicLink()).toBe(true);
  });
  ...
  it("runs prepare/build when expected artifacts are missing", async () => {
    ...
    const fresh = await import(`./package-install-finalizer.ts?missing-artifacts=${Date.now()}`);
    const result = fresh.repairConfiguredPiPackages({ cwd, agentDir, logger: { info() {}, warn() {} }, force: true });

    expect(result.built).toContain(packageRoot);
    expect(spawnCalls.some((call) => call.includes("run build"))).toBe(true);
