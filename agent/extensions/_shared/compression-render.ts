...
export function formatCompressionFooter(details: CompressionDetails, theme: Theme): string {
  const { originalLength, compressedLength, savedPct } = details;
  return theme.fg("muted", ` ${icon} • ${originalLength} → ${compressedLength} chars (-${savedPct}%)`);
}

export function readCompressionDetails(details: object | undefined): CompressionDetails | null {
