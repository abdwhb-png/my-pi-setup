...
The three tools are hidden by default. They become visible while the `herdr-orchestrator` role is active, or after the user runs `/herdr-tools on`. Use `/herdr-tools off` to remove only the manual grant and `/herdr-tools status` to inspect the current grant. Manual grants do not survive reload, resume, shutdown, or a new session. There is no agent-callable activation tool.
...
The tools are opt-in through `herdr-orchestrator` or the user-only `/herdr-tools` command. Loading the extension does not turn general background work or delegation into a Herdr workflow.
