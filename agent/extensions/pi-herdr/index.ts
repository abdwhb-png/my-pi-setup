...
const DirectionEnum = StringEnum(["right", "down"] as const, {
    description:
        "Split direction. When omitted, the tool chooses from the source pane geometry.",
});
...
function chooseSplitDirection(
    ...
    return pane.rect.width >= 80 && pane.rect.width >= pane.rect.height * 2
        ? "right"
        : "down";
}
...
export default function (pi: ExtensionAPI) {
    ...
    function parseRoleGrant(payload: unknown): boolean {
        ...
        const policy = payload as Partial<RoleToolPolicy>;
        return (
            policy.version === 1 &&
            policy.roleName === HERDR_ROLE &&
            policy.mode === "set" &&
            Array.isArray(policy.toolNames) &&
            HERDR_TOOL_NAMES.every((name) => policy.toolNames?.includes(name))
        );
    }
    ...
    pi.registerCommand("herdr-tools", {
        ...
        handler: async (args, ctx) => {
            ...
            ctx.ui.notify(`Herdr tools: ${visibilityStatus()}`, "info");
        },
    });
    ...
    async function execHerdrJson<T>(
        args: string[],
        signal?: AbortSignal,
    ): Promise<T> {
        ...
        if (!stdout)
            throw new Error(
                `Expected JSON output from herdr ${args.join(" ")}`,
            );
    ...
    pi.registerTool({
        ...
        promptSnippet:
            "Inspect or create Herdr workspaces, tabs, and pane topology",
        promptGuidelines: [
        ...
        parameters: Type.Object({
            ...
            workspace: Type.Optional(
                Type.String({ description: "Opaque workspace ID" }),
            ),
            tab: Type.Optional(Type.String({ description: "Opaque tab ID" })),
            pane: Type.Optional(
                Type.String({
                    description:
                        "Opaque source pane ID. Omit for current, pane_layout, or pane_split to use the caller's pane.",
                }),
            ),
            label: Type.Optional(
                Type.String({
                    description: "Label for a new workspace or tab",
                }),
            ),
            ...
            cwd: Type.Optional(
                Type.String({
                    description:
                        "Working directory. Defaults to the caller pane's foreground cwd.",
                }),
            ),
            focus: Type.Optional(
                Type.Boolean({
                    description:
                        "Change UI focus after creation. Defaults to false.",
                }),
            ),
        }),
        async execute(_toolCallId, params, signal, _onUpdate, _ctx) {
            switch (params.action) {
                case "current": {
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: summarizePane(pane, pane.pane_id),
                            },
                        ],
                ...
                case "workspace_create": {
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Created workspace ${workspace.workspace_id}, tab ${tab.tab_id}, root pane ${rootPane.pane_id}`,
                            },
                        ],
                        details: {
                ...
                case "workspace_focus": {
                    if (!params.workspace)
                        throw new Error(
                            "'workspace' is required for workspace_focus",
                        );
                    const response = await execHerdrJson<{
                        result: { workspace: WorkspaceInfo };
                    }>(["workspace", "focus", params.workspace], signal);
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Focused workspace ${response.result.workspace.workspace_id}`,
                            },
                        ],
                        details: {
                ...
                case "tab_create": {
                    ...
                    const args = [
                        ...
                        "create",
                        "--workspace",
                        params.workspace || current.workspace_id,
                    ];
                    args.push(
                        "--cwd",
                        params.cwd ||
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Created tab ${tab.tab_id}, root pane ${rootPane.pane_id}`,
                            },
                        ],
                ...
                case "tab_focus": {
                    if (!params.tab)
                        throw new Error("'tab' is required for tab_focus");
                    const response = await execHerdrJson<{
                        result: { tab: TabInfo };
                    }>(["tab", "focus", params.tab], signal);
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Focused tab ${response.result.tab.tab_id}`,
                            },
                        ],
                        details: {
                            action: "tab_focus",
                ...
                case "pane_list": {
                    ...
                    }>(["pane", "list", "--workspace", workspaceId], signal);
                ...
                case "pane_layout": {
                    const paneId =
                        params.pane || (await getCurrentPane(signal)).pane_id;
                ...
                case "pane_split": {
                    ...
                    const source = params.pane
                        ? await getPane(params.pane, signal)
                        : current;
                    const direction =
                        params.direction ||
                        chooseSplitDirection(
                            await getPaneLayout(source.pane_id, signal),
                            source.pane_id,
                        );
                    const cwd =
                        params.cwd ||
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Created pane ${pane.pane_id} by splitting ${source.pane_id} ${direction}`,
                            },
                        ],
                        details: {
                            action: "pane_split",
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            pane: Type.String({
                description: "Opaque pane ID returned by herdr_layout",
            }),
            ...
            text: Type.Optional(
                Type.String({
                    description:
                        "Literal text to send without Enter for send_text",
                }),
            ),
            keys: Type.Optional(
                Type.Array(Type.String(), {
                    description:
                        "Logical terminal keys for send_keys, such as esc, enter, up, or ctrl+c",
                }),
            ),
            match: Type.Optional(
                Type.String({
                    description:
                        "Literal substring or Rust regular expression for wait_output",
                }),
            ),
            regex: Type.Optional(
                Type.Boolean({
                    description: "Treat match as a Rust regular expression",
                }),
            ),
            ...
            lines: Type.Optional(
                Type.Integer({
                    minimum: 1,
                    description: "Rendered terminal rows to read or search",
                }),
            ),
            format: Type.Optional(OutputFormatEnum),
            raw: Type.Optional(
                Type.Boolean({
                    description: "Keep ANSI escapes while matching wait_output",
                }),
            ),
        ...
        async execute(_toolCallId, params, signal, onUpdate, _ctx) {
            switch (params.action) {
                ...
                case "run": {
                    ...
                    await execHerdrOk(
                        ["pane", "run", params.pane, params.command],
                        signal,
                    );
                    return {
                        content: [
                ...
                case "read": {
                    const args = [
                        ...
                        params.pane,
                        "--source",
                        params.source || "recent-unwrapped",
                    ];
                ...
                case "wait_output": {
                    if (!params.match)
                        throw new Error("'match' is required for wait_output");
                    if (params.source === "detection")
                        throw new Error(
                            "wait_output does not support the detection source; use read",
                        );
                    ...
                    onUpdate?.({
                        content: [
                            {
                                type: "text",
                                text: `Waiting for output in ${params.pane}...`,
                            },
                        ],
                        details: {
                            action: "wait_output",
                    ...
                    if (params.source)
                        args.push(
                            "--source",
                            params.source as WaitOutputSource,
                        );
                    if (params.lines != null)
                        args.push("--lines", String(params.lines));
                ...
                case "send_text": {
                    if (!params.text)
                        throw new Error("'text' is required for send_text");
                    await execHerdrOk(
                        ["pane", "send-text", params.pane, params.text],
                        signal,
                    );
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Sent literal text to pane ${params.pane}`,
                            },
                        ],
                ...
                case "send_keys": {
                    if (!params.keys?.length)
                        throw new Error("'keys' is required for send_keys");
                    await execHerdrOk(
                        ["pane", "send-keys", params.pane, ...params.keys],
                        signal,
                    );
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Sent ${params.keys.join(" ")} to pane ${params.pane}`,
                            },
                        ],
                        details: {
                            action: "send_keys",
                ...
                case "close": {
                    const current = await getCurrentPane(signal);
                    if (params.pane === current.pane_id)
                        throw new Error(
                            "Refusing to close the pane pi is running in.",
                        );
                    await execHerdrOk(["pane", "close", params.pane], signal);
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Closed pane ${params.pane}`,
                            },
                        ],
    ...
    pi.registerTool({
        ...
        promptSnippet:
            "Start, prompt, wait for, read, and interact with coding agents in Herdr",
        promptGuidelines: [
        ...
        parameters: Type.Object({
            ...
            target: Type.Optional(
                Type.String({
                    description:
                        "Unique live agent name or pane ID currently hosting the agent",
                }),
            ),
            pane: Type.Optional(
                Type.String({
                    description: "Existing available shell pane ID for start",
                }),
            ),
            name: Type.Optional(
                Type.String({
                    ...
                    description:
                        "Unique agent name for start or replacement name for rename",
                }),
            ),
            ...
            agentArgs: Type.Optional(
                Type.Array(Type.String(), {
                    description:
                        "Native agent arguments passed unchanged after -- for start",
                }),
            ),
            prompt: Type.Optional(
                Type.String({
                    description: "Prompt text submitted atomically with Enter",
                }),
            ),
            wait: Type.Optional(
                Type.Boolean({
                    description:
                        "Wait for lifecycle settlement after prompt. Defaults to true.",
                }),
            ),
            until: Type.Optional(
                Type.Array(StatusEnum, {
                    description:
                        "Accepted lifecycle states for prompt with wait or wait; defaults to idle, done, or blocked",
                }),
            ),
            timeout: Type.Optional(
                Type.Integer({
                    minimum: 1,
                    description:
                        "Timeout in milliseconds; omitted means indefinite",
                }),
            ),
            source: Type.Optional(ReadSourceEnum),
            lines: Type.Optional(
                Type.Integer({
                    minimum: 1,
                    description: "Rendered terminal rows to read",
                }),
            ),
            format: Type.Optional(OutputFormatEnum),
            keys: Type.Optional(
                Type.Array(Type.String(), {
                    description:
                        "Logical UI keys such as esc, enter, up, or ctrl+c",
                }),
            ),
            clearName: Type.Optional(
                Type.Boolean({
                    description: "Clear the current agent name for rename",
                }),
            ),
        }),
        async execute(_toolCallId, params, signal, onUpdate, _ctx) {
            switch (params.action) {
                        ...
                        content: [
                            {
                                type: "text",
                                text: agents.length
                                    ? agents.map(summarizeAgent).join("\n")
                                    : "No agents.",
                            },
                        ],
                ...
                case "get": {
                    if (!params.target)
                        throw new Error("'target' is required for get");
                    const response = await execHerdrJson<{
                        result: { agent: AgentInfo };
                    }>(["agent", "get", params.target], signal);
                    return {
                        content: [
                            {
                                type: "text",
                                text: summarizeAgent(response.result.agent),
                            },
                        ],
                        details: {
                            action: "get",
                ...
                case "start": {
                    if (!params.name)
                        throw new Error("'name' is required for start");
                    if (!params.kind)
                        throw new Error("'kind' is required for start");
                    if (!params.pane)
                        throw new Error("'pane' is required for start");
                    if (
                        params.timeout != null &&
                        (params.timeout <= 3000 || params.timeout > 300000)
                    ) {
                        throw new Error(
                            "start timeout must be greater than 3000ms and at most 300000ms",
                        );
                    }
                    const args = [
                        "agent",
                    ...
                    if (params.timeout != null)
                        args.push("--timeout", String(params.timeout));
                    if (params.agentArgs?.length)
                        args.push("--", ...params.agentArgs);
                    onUpdate?.({
                        content: [
                            {
                                type: "text",
                                text: `Starting ${params.kind} as ${params.name} in ${params.pane}...`,
                            },
                        ],
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Started ${summarizeAgent(response.result.agent)}`,
                            },
                        ],
                        details: {
                            action: "start",
                ...
                case "prompt": {
                    if (!params.target)
                        throw new Error("'target' is required for prompt");
                    if (!params.prompt)
                        throw new Error("'prompt' is required for prompt");
                    const shouldWait = params.wait !== false;
                    if (!shouldWait && params.until?.length)
                        throw new Error("'until' requires wait for prompt");
                    if (!shouldWait && params.timeout != null)
                        throw new Error("'timeout' requires wait for prompt");
                    const args = [
                        "agent",
                        "prompt",
                        params.target,
                    ...
                    for (const status of params.until || [])
                    ...
                    if (shouldWait) {
                        onUpdate?.({
                            content: [
                                {
                                    type: "text",
                                    text: `Prompted ${params.target}; waiting for lifecycle settlement...`,
                                },
                            ],
                            details: {
                                action: "prompt",
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: `${shouldWait ? "Prompt settled" : "Prompt submitted"}: ${summarizeAgent(response.result.agent)}`,
                            },
                        ],
                        details: {
                            action: "prompt",
                ...
                case "wait": {
                    if (!params.target)
                        throw new Error("'target' is required for wait");
                    ...
                    onUpdate?.({
                        content: [
                            {
                                type: "text",
                                text: `Waiting for agent ${params.target}...`,
                            },
                        ],
                        details: {
                            action: "wait",
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Agent settled: ${summarizeAgent(response.result.agent)}`,
                            },
                        ],
                        details: {
                            action: "wait",
                ...
                case "read": {
                    if (!params.target)
                        throw new Error("'target' is required for read");
                    const args = [
                        "agent",
                        "read",
                        params.target,
                    ...
                    if (params.lines != null)
                        args.push("--lines", String(params.lines));
                    if (params.format)
                        args.push("--format", params.format as OutputFormat);
                    const output = await execHerdrText(args, signal);
                    return {
                        content: [{ type: "text", text: formatOutput(output) }],
                        details: {
                ...
                case "send_keys": {
                    if (!params.target)
                        throw new Error("'target' is required for send_keys");
                    if (!params.keys?.length)
                        throw new Error("'keys' is required for send_keys");
                    await execHerdrOk(
                        ["agent", "send-keys", params.target, ...params.keys],
                        signal,
                    );
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Sent ${params.keys.join(" ")} to ${params.target}`,
                            },
                        ],
                        details: {
                            action: "send_keys",
                ...
                case "focus": {
                    if (!params.target)
                        throw new Error("'target' is required for focus");
                    const response = await execHerdrJson<{
                        result: { agent: AgentInfo };
                    }>(["agent", "focus", params.target], signal);
                    return {
                        content: [
                            {
                                type: "text",
                                text: `Focused ${agentDisplayName(response.result.agent)}`,
                            },
                        ],
                        details: {
                            action: "focus",
                ...
                case "rename": {
                    if (!params.target)
                        throw new Error("'target' is required for rename");
                    if (!params.clearName && !params.name)
                        throw new Error(
                            "'name' or 'clearName' is required for rename",
                        );
                    ...
                    return {
                        content: [
                            {
                                type: "text",
                                text: params.clearName
                                    ? `Cleared agent name for ${params.target}`
                                    : `Renamed agent to ${params.name}`,
                            },
                        ],
                        details: {
                            action: "rename",
