...
export function registerCronCommand(
    ...
    pi.registerCommand("cron", {
        description:
            "Schedule, list, or delete recurring prompts (e.g., /cron 5m check deploy)",
        getArgumentCompletions: (prefix: string) => {
            ...
            if (parts.length <= 1 && !endsWithSpace) {
                const filtered = subcommands.filter((value) =>
        ...
        handler: async (args, ctx) => {
            ...
            if (!input || input === "help" || input === "?") {
                ...
                ctx.ui.notify(
                    "Cron usage:\n\n" +
                        ...
                        "  /cron show <id>          inspect a task\n" +
                        "  /cron delete <id>        cancel a task\n" +
                        "  /cron remove <id>        cancel a task\n" +
            ...
            if (deleteMatch) {
                ...
                if (scheduler.deleteTask(id)) {
                    ...
                    ctx.ui.notify(
                        `Task ${id} not found. Use /loop list`,
                        "error",
                    );
                }
                return;
            ...
            if (input === "clear" || input === "clear all") {
                if (store.size() > 0 && ctx.hasUI) {
                    const confirmed = await ctx.ui.confirm(
                        ...
                        `Delete ${store.size()} scheduled cron task(s)?`,
                    );
                    if (!confirmed) {
            ...
            if (store.size() >= MAX_TASKS) {
                ctx.ui.notify(
                    `Maximum ${MAX_TASKS} tasks. Use /cron delete <id> or /cron clear`,
                    "error",
                );
                return;
            }
            ...
            if (!parsed || !parsed.prompt) {
                ctx.ui.notify(
                    "Could not parse. Usage: /loop [interval] <prompt>",
                    "error",
                );
                return;
            }
            ...
            let msg =
                `Scheduled ${task.id} (${parsed.humanLabel})\n` +
