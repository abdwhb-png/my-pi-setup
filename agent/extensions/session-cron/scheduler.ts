...
import {
    releaseCronSchedulerLock,
    tryAcquireCronSchedulerLock,
} from "./cron-lock";
...
export class LoopScheduler {
    ...
    async start(): Promise<void> {
        ...
        this.isOwner = await tryAcquireCronSchedulerLock(this.sessionId).catch(
            () => false,
        );
        if (this.isOwner) {
        ...
        this.lockProbeTimer = setInterval(() => {
            void tryAcquireCronSchedulerLock(this.sessionId)
                .then(async (owned) => {
    ...
    async stop(): Promise<void> {
        ...
        this.isOwner = false;
        if (wasOwner) {
            await releaseCronSchedulerLock(this.sessionId).catch(
                () => undefined,
            );
        }
    }
    ...
    fireDueTasks(): void {
        ...
        for (const task of this.store.list()) {
            ...
            if (!task.recurring || isExpired) {
                ...
                const newNext =
                    computeTaskNextFireAt(
                        task,
                        now,
                        task.jitterMs,
                        task.cron,
                    ) ?? now + FALLBACK_FIRE_DELAY_MS;
