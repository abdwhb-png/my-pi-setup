...
import { releaseCronSchedulerLock, tryAcquireCronSchedulerLock } from "./cron-lock";
...
export class LoopScheduler {
	...
	async start(): Promise<void> {
		...
		this.isOwner = await tryAcquireCronSchedulerLock(this.sessionId).catch(() => false);
		if (this.isOwner) {
	...
	async stop(): Promise<void> {
		...
		this.isOwner = false;
		if (wasOwner) {
			await releaseCronSchedulerLock(this.sessionId).catch(() => undefined);
		}
	}
