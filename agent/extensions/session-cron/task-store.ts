...
export class InMemoryTaskStore implements TaskStore {
    ...
    replaceAll(
        ...
        immediateIds: Set<string> = new Set(),
    ): void {
        this.clear();
...
export function buildRuntimeTask(
    parsed: ParsedInterval,
    recurring: boolean,
): RuntimeCronTask {
    const id = generateId();
...
export function hydrateRuntimeTask(
    ...
    const nextFireAt = options.forceImmediate
        ...
        : (computeTaskNextFireAt(
              ...
              jitterMs,
              cron,
          ) ?? Date.now() + FALLBACK_FIRE_DELAY_MS);
...
export function findMissedTasks(
    ...
    return tasks.filter((task) => {
        ...
        const nextFireAt = computeTaskNextFireAt(
            task,
            task.lastFiredAt ?? task.createdAt,
        );
...
export function computeTaskNextFireAt(
    ...
    const cron =
        precomputedCron ??
        new Cron(task.cronExpr, {
            timezone: Intl.DateTimeFormat().resolvedOptions().timeZone,
        });
    const periodMs = estimatePeriodMs(task.cronExpr);
    const jitterMs =
        precomputedJitterMs ??
        computeJitterMs(task.id, periodMs, task.recurring);
...
function computeJitterMs(
    ...
    if (recurring) {
        const maxJitter = Math.min(
            periodMs * RECURRING_JITTER_FRAC,
            RECURRING_JITTER_CAP_MS,
        );
