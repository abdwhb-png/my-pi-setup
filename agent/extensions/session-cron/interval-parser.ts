...
export function estimatePeriodMs(expr: string): number {
    ...
    if (minute.startsWith("*/"))
        return parseInt(minute.slice(2), 10) * MS_PER_MINUTE;
...
function intervalToCron(
    ...
    switch (unit) {
        case "s": {
            ...
            rounded =
                value < 60
                    ? `Rounded ${value}s up to ${minutes}m (cron minimum is 1 minute)`
                    : undefined;
...
function minutesToCron(
    ...
    const nearestHours =
        24 % clampedHours !== 0
            ? findNearestDivisor(clampedHours, 24)
            : clampedHours;
