...
export default function sessionCronExtension(pi: ExtensionAPI) {
    ...
    async function startFileWatcher(): Promise<void> {
        ...
        try {
            ...
            fileWatcher = watch(tasksDir, (_eventType, changedFile) => {
                if (changedFile !== null && changedFile !== tasksFileName)
                    return;
    ...
    async function restoreOwnedTasks(ctx: ExtensionContext): Promise<void> {
        ...
        const restoredTasks = storedTasks.filter(
            (task) => !declinedOneShots.has(task.id),
        );
        ...
        if (ctx.hasUI && missedTasks.length > 0) {
            const recurringCount = missedTasks.filter(
                (task) => task.recurring,
            ).length;
            ...
            if (oneShotCount > 0) {
                parts.push(
                    `${oneShotCount} one-shot task${oneShotCount === 1 ? "" : "s"} reviewed`,
                );
            }
            ctx.ui.notify(
                `Recovered missed loop tasks: ${parts.join(", ")}`,
                "info",
            );
        }
    }
