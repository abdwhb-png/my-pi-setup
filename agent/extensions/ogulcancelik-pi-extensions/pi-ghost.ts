...
class GhostOverlayComponent implements Focusable {
    ...
    private getTranscriptLines(width: number): string[] {
        ...
        const lines = stripPromptMarkers(
            this.transcriptContainer.render(width),
        );
    ...
    render(width: number): string[] {
        ...
        if (this.followMode) {
            ...
            this.scrollOffset = Math.max(
                0,
                Math.min(this.scrollOffset, maxScroll),
            );
            if (this.scrollOffset >= maxScroll) this.scrollToBottom();
        }
        ...
        const headerContent = truncateToWidth(
            th.fg("accent", ` ${th.bold("ghost pi")} `) +
                th.fg(
                    "dim",
                    "same model • ephemeral session • ↑↓ scroll • ctrl+s hide • esc close",
                ),
            innerW,
        );
...
function formatGhostCwd(cwd: string): string {
    ...
    if (home && cwd.startsWith(home)) {
        const relativeCwd = cwd.slice(home.length);
        return relativeCwd ? `~${relativeCwd}` : "~";
    }
    return cwd;
}
...
export default function (pi: ExtensionAPI) {
    ...
    const cleanupGhost = (ctx?: ExtensionContext) => {
        overlayClosed = true;
        if (overlayHandle) {
            try {
                overlayHandle.hide();
                ...
                (_tui, theme) => ({
                    render: () => [
                        theme.fg("accent", "/gpi ") +
                            theme.fg(
                                "dim",
                                "is running • run /gpi to bring it back",
                            ),
                    ],
                    invalidate: () => {},
                }),
            ...
            .custom<void>(
                (tui, theme, keybindings, done) => {
                    ...
                    if (initialPrompt?.trim()) {
                        void session.prompt(initialPrompt.trim(), {
                            images: [],
                        });
                    }
