...
function getArgumentCompletions(prefix: string) {
	...
	const options = [
		...
		{ value: "--show", label: "--show", description: "Show current thinking state" },
		{ value: "--reset", label: "--reset", description: "Clear remembered thinking levels" },
	];
	const filtered = options.filter((option) => option.value.startsWith(trimmed));
...
function setModelThinking(pi: ExtensionAPI, ctx: ExtensionContext, level: ThinkingLevel): void {
	const model = ctx.model;
	if (!model) {
...
export default function modelThinkingExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("model-thinking", {
		description: "Set model-specific thinking levels (/model-thinking <mode>|--show|--reset)",
		...
		handler: async (args, ctx) => {
			...
			if (isThinkingLevel(cmd)) {
				setModelThinking(pi, ctx, cmd);
				return;
			}
