...
import { buildSkillList, findSkill, searchSkills, type SkillEntry } from "./skill-index";
...
export default function piSkillLoader(pi: ExtensionAPI): void {
  ...
  const searchSkillTool = defineTool({
    ...
    parameters: Type.Object({
      query: Type.String({ description: "Search query — matches against skill name and description" }),
    }),
    async execute(
      _toolCallId: string,
      params: { query: string },
      ...
      if (results.length === 0) {
        return {
          content: [{ type: "text", text: "No skills found matching your query. Try a different search term." }],
          details: undefined,
        };
      }

      const lines = [`Found ${results.length} skill(s) matching "${params.query}":`, ""];
      for (const s of results) {
        lines.push(`  • ${s.name} (${s.source})`);
        if (s.description) lines.push(`    ${s.description}`);
      }
      lines.push("", `Use load_skill("${results[0].name}") to load a skill's full instructions.`);
  ...
  const findSkillTool = defineTool({
    ...
    parameters: Type.Object({
      name: Type.String({ description: "Exact skill name to look up (case-insensitive)" }),
    }),
    async execute(
      _toolCallId: string,
      params: { name: string },
      ...
      if (!skill) {
        return {
          content: [{ type: "text", text: `Skill "${params.name}" not found. Use search_skill to discover available skills.` }],
          details: undefined,
        };
      }
  ...
  const loadSkillTool = defineTool({
    ...
    parameters: Type.Object({
      name: Type.String({ description: "Exact skill name to load (case-insensitive)" }),
    }),
    async execute(
      _toolCallId: string,
      params: { name: string },
      ...
      try {
        ...
        return {
          content: [{ type: "text", text: `Failed to read skill file at ${skill.path}: ${err instanceof Error ? err.message : String(err)}` }],
          details: undefined,
        };
      }
  ...
  pi.on("session_start", () => {
    ...
    pi.registerCommand("load-skills", {
      ...
      getArgumentCompletions: (prefix: string): { value: string; label: string; description?: string }[] => {
        ...
        const alreadyMatched = new Set(parts.slice(0, -1).filter(Boolean));
    ...
    const added = ["search_skill", "find_skill", "load_skill"].filter((t) => !current.includes(t));
    if (added.length > 0) {
