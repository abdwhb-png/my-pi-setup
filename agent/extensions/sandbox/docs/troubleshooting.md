...
| Docker authorization is enabled but the footer says CLI unavailable | Run `/sandbox doctor docker`. Authorize the executable's PATH location and canonical target explicitly. Docker broker permission alone exposes neither file. |
| CLI exposed but Compose unavailable | Run `/sandbox doctor` and inspect the plugin path and canonical target. Grant the discoverable plugin path, target and dependencies explicitly. A plugin elsewhere in an authorized installation is not automatically discoverable. |
| Docker client check pending or not verified | Before admission, use `/sandbox doctor` for planned inspection. An admitted unverified result includes the inspection failure or unsupported custom configuration. Exposure does not prove execution or daemon readiness. |
