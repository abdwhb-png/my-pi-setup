...
Authorize the Docker client and Compose plugin through an explicit installation or filesystem grant. Docker authorization does not install or expose either command. Include both the discoverable path and any canonical symlink target, plus required dependencies. Adding a canonical installation root does not expose an ungranted executable link in another directory or put that root's commands on PATH.

The footer keeps the Docker authorization summary separate from client exposure. It shows `client check pending` before a validated Bash admission. After admission, static inspection of the admitted PATH and read scope can show `CLI unavailable`, `CLI exposed · Compose unavailable`, `CLI exposed · Compose exposed`, or an explicit unverified state when inspection cannot establish exposure. Host mode and disabled Docker omit this sandbox client check. `/sandbox status` shows the most recent admitted inspection; `/sandbox doctor` also supports a planned inspection before any shell process starts. Neither inspection executes Docker, contacts its daemon, grants access, or retries a command.

Compose inspection covers the private HOME plugin directory and the standard Linux `/usr/local/lib`, `/usr/local/libexec`, `/usr/lib`, and `/usr/libexec` Docker plugin directories. A custom `DOCKER_CONFIG` remains unverified. Static ELF and interpreter inspection cannot prove dynamic loading, arbitrary custom plugin discovery, runtime execution, or daemon readiness. Client inspection updates on Bash admission and sandbox status changes; it is not an independent live monitor of installation contents.
