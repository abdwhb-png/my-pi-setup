...
test.skipIf(process.platform !== "linux" || !hasCandidateRuntime())("real isolated Bash cannot reach a host listener or overwrite its authority", async () => {
    ...
    const service = createSandboxService({
        ...
        // curl is deliberately opened only to prove the engine, rather than a
        // missing command, blocks this host-listener request.
        config: validatePiSandboxConfig({ filesystem: { allowRead: await hostToolReadClosure("/usr/bin/curl"), allowWrite: ["."], denyRead: [future], denyWrite: [future] } }),
    });
    ...
    try {
        ...
        expect((await operations.exec("/usr/bin/curl --version >/dev/null", cwd, { timeout: 5, onData() {} })).exitCode).toBe(0);
        for (const command of [`/usr/bin/curl --fail --silent --max-time 2 http://127.0.0.1:${address.port}`, "printf overwritten > sandbox.json"]) {
