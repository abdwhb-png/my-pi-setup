...
The extension owns one process-global execution broker shared by `bash`, `safe_bash`, and interactive user Bash. Enabled state uses sandbox operations. Explicit disabled state uses local operations. Missing, uninitialized, unsupported, or failed state blocks execution instead of silently falling back locally.
