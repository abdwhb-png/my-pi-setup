 ...
 *
 * Tools:
 * 1. pi_session_search - Find relevant past sessions by keywords.
 *    Use when the user mentions something from a previous conversation
 *    and you need to find which session it was in.
 *
 * 2. pi_session_query - Deep dive into a specific session.
 *    Use after pi_session_search to get detailed information from a
 *    specific session, or when you already know the session path
 *    (e.g., from a handoff).
 *
 * Command:
 * /pi-session-recall - Configure the model used for session queries.
 *    Shows all available models and lets you pick one. Persists the
 *    choice to ~/.pi/agent/pi-session-recall.json (or equivalent).
 *
 * Typical flow: user says "remember when we tried X?" →
 *   pi_session_search("X") → find the right session →
...
function getConfigPath(): string {
    return join(getAgentDir(), "pi-session-recall.json");
}
...
function loadConfig(): SessionRecallConfig {
    ...
    const readablePath = existsSync(configPath)
        ? configPath
        : getLegacyConfigPath();
    if (!existsSync(readablePath)) return {};
    try {
        return JSON.parse(readFileSync(readablePath, "utf-8"));
    } catch {
        return {};
    }
}
...
/**
 * Resolve query model candidates from config → fallback models → current session model.
 */
function resolveQueryModelCandidates(ctx: ExtensionContext): Model<Api>[] {
    const config = loadConfig();
    const candidates: Model<Api>[] = [];
    const addConfiguredModel = (
        modelConfig: { provider: string; id: string } | undefined,
    ) => {
...
> {
    let lastError = "No model available to analyze the session.";
    for (const model of resolveQueryModelCandidates(ctx)) {
...
function buildWindowedContext(
    ...
    for (let i = Math.max(0, total - BOOKEND_COUNT); i < total; i++)
        included.add(i);
    ...
    const currentChars = () =>
        [...included].reduce(
            (sum, i) => sum + serializedMessages[i].charCount + 20,
            0,
        );
    ...
    if (currentChars() < charBudget * 0.8) {
        for (
            let i = BOOKEND_COUNT;
            i < total && currentChars() < charBudget * 0.8;
            i++
        ) {
            included.add(i);
        }
...
function sessionResultFromPath(
    filePath: string,
    count: number,
): SessionSearchResult {
    return {
...
function findSessionsById(
    ...
    return walkJsonlFiles(sessionsDir)
        .filter(
            (filePath) =>
                parseSessionIdFromPath(filePath)?.toLowerCase() ===
                normalizedId,
        )
...
function searchSessionsByQuery(
    query: string,
    sessionsDir: string,
): SessionSearchResult[] {
...
function buildSessionSearchOutput(
...
export default function sessionRecallExtension(pi: ExtensionAPI) {
    // ── /pi-session-recall command ─────────────────────────────────────────

    pi.registerCommand("pi-session-recall", {
        description: "Configure the model used for session queries",
        handler: async (_args, ctx) => {
            if (!ctx.hasUI) {
                ctx.ui.notify(
                    "Interactive mode required for /pi-session-recall",
                    "warning",
                );
                return;
            }
            ...
            const choice = await ctx.ui.custom<string | undefined>(
                (tui, theme, _kb, done) => {
                    ...
                    const title = new Text(
                        theme.bold("Query model for pi_session_query") +
                    ...
                    const rebuildList = () => {
                        ...
                        const spacerIdx =
                            container.children.indexOf(listSpacer);
                        container.children.splice(spacerIdx + 1, 0, list);
                        tui.requestRender();
                    };

                    const interactiveContainer = container as Component;
                    interactiveContainer.handleInput = (data: string) => {
            ...
            if (choice === SESSION_MODEL_VALUE) {
                ...
                saveConfig(config);
                ctx.ui.notify(
                    "Pi session query will use the current session model",
                    "info",
                );
            } else {
                ...
                saveConfig(config);
                ctx.ui.notify(
                    `Pi session query model set to ${choice}`,
                    "info",
                );
            }
        },
    ...
    pi.registerTool({
        ...
        description:
            ...
            "Do not combine independent keywords into one search string. If you need multiple unrelated terms, call pi_session_search multiple times. " +
            "After finding a likely session, use pi_session_query for semantic questions.",
        renderResult: (result, options, theme) => {
            ...
            const details = result.details as
                | { matchCount?: number; query?: string; error?: boolean }
                | undefined;
                    ...
                    content: [
                        {
                            ...
                            text:
                                ...
                                "pi_session_search is literal fixed-string search, not semantic search. Retry with one exact distinctive token or phrase, such as a filename, package name, error text, function name, issue id, or unique term.",
                        },
                    ],
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            sessionId: Type.String({
                description:
                    "Exact session id UUID from a pi session filename.",
            }),
        }),
            ...
            if (results.length === 0) {
                return {
                    content: [
                        {
                            type: "text" as const,
                            text: `No session found for id "${sessionId}".`,
                        },
                    ],
            ...
            const output = results
                .map((result) => {
                    const foundSessionId =
                        parseSessionIdFromPath(result.filePath) ?? sessionId;
    ...
    // ── pi_session_query tool ──────────────────────────────────────────────

    pi.registerTool({
        name: "pi_session_query",
        label: "Session Query",
        description:
            "Query a specific session file to get detailed information. Use after pi_session_search to dig into a particular session, " +
            ...
            if (options.expanded) {
                ...
                const summary =
                    firstLine.length > 120
                        ...
                        : firstLine;
                container.addChild(
                    new Text(theme.fg("toolOutput", summary), 0, 0),
                );
            }
        ...
        async execute(_toolCallId, params, signal, onUpdate, ctx) {
            ...
            // Resolve query model: user config → fallback models → current session model
            const queryModelAuth = await resolveAuthedQueryModel(ctx);
            if (!queryModelAuth.ok) {
                return errorResult(
                    `Error resolving model auth: ${queryModelAuth.error}. ` +
                        "Configure one with /pi-session-recall or set up API keys.",
                );
            }
            const queryModel = queryModelAuth.model;
            ...
            if (fullTokens <= tokenBudget) {
                ...
                conversationText = buildWindowedContext(
                    serialized,
                    question,
                    tokenBudget,
                );
            ...
            try {
                ...
                const response = await complete(
                    ...
                    {
                        apiKey: queryModelAuth.apiKey,
                        headers: queryModelAuth.headers,
                        signal,
                    },
                );
