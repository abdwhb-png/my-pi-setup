...
function loadConfig(): SessionRecallConfig {
	...
	const readablePath = existsSync(configPath) ? configPath : getLegacyConfigPath();
...
function resolveQueryModelCandidates(ctx: ExtensionContext): Model<Api>[] {
	...
	const addConfiguredModel = (modelConfig: { provider: string; id: string } | undefined) => {
		if (!modelConfig) return;
		const found = ctx.modelRegistry.find(modelConfig.provider, modelConfig.id);
		if (found) candidates.push(found);
	};

	addConfiguredModel(config.queryModel);
	for (const fallback of config.fallbackModels ?? config["fallback-models"] ?? []) {
...
export default function sessionRecallExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("pi-session-recall", {
		...
		handler: async (_args, ctx) => {
			if (!ctx.hasUI) {
				ctx.ui.notify("Interactive mode required for /pi-session-recall", "warning");
				return;
			}
			...
			if (choice === SESSION_MODEL_VALUE) {
				...
				saveConfig(config);
				ctx.ui.notify("Pi session query will use the current session model", "info");
			} else {
				...
				saveConfig(config);
				ctx.ui.notify(`Pi session query model set to ${choice}`, "info");
			}
		},
	});
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			try {
				...
				const response = await complete(
					...
					{ apiKey: queryModelAuth.apiKey, headers: queryModelAuth.headers, signal },
				);
