...
import type { CompressRequest, CompressResponse, ToolResultHandlerOptions } from "./types";
...
export function chooseCompressionRoute(input: {
  ...
  if (input.toolName === "grep" || input.toolName === "bash" || input.toolName === "safe_bash" || input.toolName === "ls" || input.toolName === "find") {
    return "cap";
  }
  return "edgee";
}
 ...
 */
export function toolCompressionContext(toolName: string): "search" | "read" | "shell" | null {
...
function withTimeout<T>(promise: Promise<T>, timeoutMs: number, signal?: AbortSignal): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    ...
    const timer: ReturnType<typeof setTimeout> = setTimeout(() => reject(new Error(`Timed out after ${timeoutMs}ms`)), timeoutMs);
    const abort = () => {
      clearTimeout(timer);
...
async function maybeCreateArchivedCap(
  ...
  if (!targetBytes || targetBytes <= 0 || text.length <= targetBytes || !options?.archiveOriginal) return null;
...
export function createToolResultHandler(options?: ToolResultHandlerOptions) {
  ...
  return async (event: ToolResultEvent, signal?: AbortSignal) => {
    ...
    try {
      ...
      const outputText = archiveNote && result.compressed_output.length + archiveNote.length < originalLength
        ? `${result.compressed_output}${archiveNote}`
...
export function summarizeToolSubject(toolName: string, input: object | undefined): string | undefined {
