...
const DEFAULT_SETTINGS: CavemanSettings = { defaultLevel: "full", showStatus: true };
let saveSettingsQueue: Promise<void> = Promise.resolve();

async function saveCavemanSettings(updates: Partial<CavemanSettings>): Promise<void> {
	...
	saveSettingsQueue = saveSettingsQueue.then(async () => {
		...
		const currentSaveTokens = (settings.saveTokens as Record<string, unknown>) ?? {};
		const currentCaveman = (currentSaveTokens.caveman as Record<string, unknown>) ?? {};
...
export default function caveman(pi: ExtensionAPI) {
	...
	async function openConfig(ctx: ExtensionContext) {
		...
		await ctx.ui.custom((_tui, theme, _kb, done) => {
			...
			container.addChild(new Text(theme.fg("dim", " Saved to settings.json"), 0, 0));
			...
			const applySettingChange = (id: string, newValue: string) => {
				...
				saveCavemanSettings({ defaultLevel: settings.defaultLevel, showStatus: settings.showStatus });
				syncStatus(ctx);
			};
