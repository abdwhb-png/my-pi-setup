...
export async function showReview(
  ...
  const { matchesKey, Key, decodeKittyPrintable } = await import("@earendil-works/pi-tui");
  ...
  return ctx.ui.custom<ReviewResult>((tui, theme, _kb, done) => {
    ...
    function handleReasonInput(data: string) {
      if (matchesKey(data, Key.enter)) {
        ...
        done({ action: "rejected", reason: reason.length > 0 ? reason : "(no reason given)" });
        return;
      }
    ...
    function handleInput(data: string) {
      ...
      if (data === "n" || matchesKey(data, Key.escape) || matchesKey(data, Key.ctrl("c"))) {
        done("reject");
        return;
      }
      if (data === "r") {
    ...
    function render(width: number): string[] {
      ...
      if (reasonMode) {
        add(colors.warning(" ⚠ Reject reason — Enter submit • Esc cancel • Backspace delete"));
        add(` > ${reasonBuffer}`);
      } else {
        let hints = "y/Enter approve • n/Esc reject • r reject w/ reason • a auto-accept (this turn)";
