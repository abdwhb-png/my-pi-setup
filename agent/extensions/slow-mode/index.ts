...
export default function slowMode(pi: ExtensionAPI) {
  ...
  async function reviewEdit(
    ...
    if (!patches) return { approved: true, editedContent: null, originalNewText: null, wroteDirectly: false, autoAccept: false, reason: null };
    ...
    if (patches.length > 1) {
      try {
        ...
        if (!extracted) return { approved: true, editedContent: null, originalNewText: null, wroteDirectly: false, autoAccept: false, reason: null };
    ...
    return { approved, editedContent, originalNewText: newText, wroteDirectly, autoAccept: autoAcceptVal, reason };
  }
