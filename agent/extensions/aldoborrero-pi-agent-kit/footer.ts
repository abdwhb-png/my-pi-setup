...
function buildFooterState(
  ctx: {
    sessionManager: {
      getBranch(): Array<{ type: string; message?: { role?: string; usage?: { input: number; output: number; cacheRead: number; cacheWrite: number; cost?: { total?: number } }; stopReason?: string } }>;
