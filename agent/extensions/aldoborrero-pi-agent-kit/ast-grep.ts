...
function runAstGrep(args: string[], cwd: string): Promise<AstGrepMatch[]> {
    return new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            ...
            if (code !== 0 && code !== 1) {
                reject(
                    new Error(stderr || `ast-grep exited with code ${code}`),
                );
                return;
            }
    ...
    for (const [file, fileMatches] of byFile) {
        ...
        for (const match of fileMatches) {
            ...
            if (match.labels && match.labels.length > 0) {
                output +=
                    "Labels: " +
                    match.labels.map((l) => l.text).join(", ") +
                    "\n";
            }
...
const TOOL_DESCRIPTION = `Search code using AST patterns with ast-grep. More powerful than text search because it understands code structure.
...
export default function (pi: ExtensionAPI) {
    pi.registerTool({
        ...
        promptSnippet:
            "Search code by structure (AST patterns). Prefer over grep for finding functions, classes, imports, and code patterns.",
        promptGuidelines: [
        ...
        parameters: Type.Object({
            ...
            context: Type.Optional(
                Type.Boolean({
                    description:
                        "Show surrounding context lines (default: false)",
                }),
            ),
            limit: Type.Optional(
                Type.Number({
        ...
        async execute(_toolCallId, params, _signal, onUpdate, ctx) {
            ...
            try {
                    ...
                    if (!p.pattern) {
                        return {
                            content: [
                                {
                                    type: "text",
                                    text: "Error: `pattern` parameter is required for pattern mode.",
                                },
                            ],
                            isError: true,
                            details: undefined,
                ...
                if (p.mode === "rule") {
                    ...
                    const { dir, filePath } = writeRuleToTempFile(
                        p.lang,
                        p.rule,
                    );
                    ...
                    if (!p.pattern) {
                        return {
                            content: [
                                {
                                    type: "text",
                                    text: "Error: `pattern` parameter is required for inspect mode.",
                                },
                            ],
                            isError: true,
                            details: undefined,
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `Error: Unknown mode "${p.mode}". Use "pattern", "rule", or "inspect".`,
                        },
                    ],
                    isError: true,
                    details: undefined,
                ...
                return {
                    content: [
                        { type: "text", text: `ast-grep failed: ${msg}` },
                    ],
                    isError: true,
                    details: undefined,
                };
        ...
        renderCall(args, theme) {
            ...
            if (p.mode === "pattern") {
                ...
                text +=
                    colors.primary(`inspect:${p.inspect_format ?? "ast"}`) +
                    " " +
                    colors.meta(p.pattern ?? "");
            }
