...
async function selectSummaryModel(
	...
	if (ctx.model.provider === "anthropic") {
		...
		if (haikuModel) {
			const resolved = await ctx.modelRegistry.getApiKeyAndHeaders(haikuModel);
...
export default function loopExtension(pi: ExtensionAPI): void {
	...
	pi.on("session_before_compact", async (event, ctx) => {
		...
		try {
			const compaction = await compact(event.preparation, ctx.model, apiKey.apiKey, undefined, instructionParts, event.signal);
			return { compaction };
		} catch (error) {
			if (ctx.hasUI) {
