...
export class UsageInlineView implements Component {
    ...
    handleInput(data: string): void {
        if (data === "\x1b" || data.toLowerCase() === "q" || data === "\r") {
            this.config.done();
        }
    }
    ...
    private rebuild(width: number): void {
        ...
        lines.push(
            muted("Messages: ") +
                ...
                muted(
                    ` (in ${window.totalInput.toLocaleString()} / out ${window.totalOutput.toLocaleString()} / cached ${window.totalCacheRead.toLocaleString()})`,
                ),
        );
        ...
        // Header
        const hdr =
            padCell("Model", modelWidth, "left") +
            gap +
            padCell("Msgs", 5, "right") +
            gap +
            padCell("In", 9, "right") +
            gap +
            padCell("Out", 9, "right") +
            gap +
            padCell("Total", 9, "right") +
            gap +
        ...
        for (const m of sorted) {
            const row =
                padCell(`${m.provider}/${m.model}`, modelWidth, "left") +
                gap +
                padCell(m.messageCount.toLocaleString(), 5, "right") +
                gap +
                padCell(m.input.toLocaleString(), 9, "right") +
                gap +
                padCell(m.output.toLocaleString(), 9, "right") +
                gap +
                padCell(m.totalTokens.toLocaleString(), 9, "right") +
                gap +
        ...
        lines.push(theme.fg("border", "  " + "─".repeat(sepWidth)));
        const totalRow =
            padCell("TOTAL", modelWidth, "left") +
            gap +
            padCell(window.totalMessages.toLocaleString(), 5, "right") +
            gap +
            padCell(window.totalInput.toLocaleString(), 9, "right") +
            gap +
            padCell(window.totalOutput.toLocaleString(), 9, "right") +
            gap +
            padCell(window.totalTokens.toLocaleString(), 9, "right") +
            gap +
