...
export function isWithinWindow(
    ...
    return (
        record.timestamp.getTime() >= now.getTime() - days * 24 * 60 * 60 * 1000
    );
}
...
export function groupByModel(
    records: UsageRecord[],
): Map<string, ModelUsageAggregate> {
...
export function computeWindow(
    ...
    const windowRecords = records.filter((r) =>
        isWithinWindow(r, days, cutoff),
    );
    const grouped = groupByModel(windowRecords);

    const models = Array.from(grouped.values()).toSorted(
        (a, b) => b.cost - a.cost,
    );
    ...
    const totalCost =
        Math.round(models.reduce((sum, m) => sum + m.cost, 0) * 100) / 100;
...
export function computeAllWindows(
    ...
    return TIME_WINDOWS.map((days) =>
        computeWindow(records, days, pricing, now),
    );
}
