...
export class UsageReportWidget implements Component {
    ...
    handleInput(data: string): void {
        ...
        const isLeft =
            data === "ArrowLeft" || data === "\x1b[D" || data === "h";
        const isRight =
            data === "ArrowRight" || data === "\x1b[C" || data === "l";
    ...
    render(width: number): string[] {
        ...
        // ── Scrollable content ──
        box.setContent(
            this.renderReportContent(report, activeTab, box.getContentWidth()),
        );
    ...
    private renderReportContent(
        ...
        const windows = report.windows.filter((w) =>
            targetDays.includes(w.days),
        );
        ...
        for (const window of windows) {
            ...
            if (window.models.length === 0) {
                lines.push(
                    theme.fg("muted", "  No usage data in this period."),
                );
                continue;
            }
            ...
            // Header row
            const hdr =
                this.cell("Source", COL.source, "left") +
                gap +
                this.cell("Model", COL.model, "left") +
                gap +
                this.cell("Msgs", COL.msgs, "right") +
                gap +
                this.cell("Input", COL.input, "right") +
                gap +
                this.cell("Output", COL.output, "right") +
                gap +
                this.cell("Cached", COL.cached, "right") +
                gap +
                this.cell("Total", COL.total, "right") +
                gap +
            ...
            for (const m of sorted) {
                const row =
                    this.cell(m.provider, COL.source, "left") +
                    gap +
                    this.cell(m.model, COL.model, "left") +
                    gap +
                    this.cell(
                    ...
                    gap +
                    this.cell(m.input.toLocaleString(), COL.input, "right") +
                    gap +
                    this.cell(m.output.toLocaleString(), COL.output, "right") +
                    gap +
                    this.cell(
            ...
            lines.push(theme.fg("border", "  " + "─".repeat(sepWidth)));
            const tot =
                this.cell("TOTAL", COL.source, "left") +
                gap +
                this.cell("", COL.model, "left") +
                gap +
                this.cell(
                    window.totalMessages.toLocaleString(),
                    COL.msgs,
                    "right",
                ) +
                gap +
                this.cell(
                    window.totalInput.toLocaleString(),
                    COL.input,
                    "right",
                ) +
                gap +
                this.cell(
                    window.totalOutput.toLocaleString(),
                    COL.output,
                    "right",
                ) +
                gap +
                this.cell(
                    window.totalCacheRead.toLocaleString(),
                    COL.cached,
                    "right",
                ) +
                gap +
                this.cell(
                    window.totalTokens.toLocaleString(),
                    COL.total,
                    "right",
                ) +
                gap +
