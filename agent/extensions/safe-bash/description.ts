 ...
 */
export function buildSafeBashDescription(input: SafeBashDescriptionInput): string {
    ...
    const defaultDenyCount = DANGER_GROUP_IDS.length - allow.length - ask.length - explicitDeny.length;

    const guardParts: string[] = [];
    if (allow.length > 0) guardParts.push(`allow=[${allow.join(",")}]`);
    if (ask.length > 0) guardParts.push(`ask=[${ask.join(",")}]`);
    if (explicitDeny.length > 0) guardParts.push(`deny=[${explicitDeny.join(",")}]`);
    if (defaultDenyCount > 0) guardParts.push(`deny(default)=${defaultDenyCount}`);
    ...
    const bypass = config.allowedShellCommands.length > 0
    ...
    const guidance = "Denied groups are blocked; ask groups require user confirm; use write/edit not rm.";

    return `${SAFE_BASH_BASE_DESCRIPTION} ${modePart}. Guard: ${guardSummary}. AllowedShell: ${bypass}. ${nativePart}. ${guidance}`;
}

export function buildSafeBashPromptSnippet(input: SafeBashDescriptionInput): string {
