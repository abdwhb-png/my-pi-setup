...
import { loadPlannotatorConfig, resolvePlanFileDir } from "@plannotator/pi-extension/config.js";
...
export default function aldoborreroExtensions(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const result = writePlan(p.path ?? "", ctx.cwd, planDir, p.content ?? "");
      return {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      // oxlint-disable-next-line typescript/no-unsafe-type-assertion -- pi tool params are unknown by API contract
      const p = params as { path?: string; edits?: Array<{ oldText: string; newText: string }> };
      const cfg = loadPlannotatorConfig(ctx.cwd);
      const planDir = resolvePlanFileDir(cfg.config);
      const result = editPlan(p.path ?? "", ctx.cwd, planDir, p.edits ?? []);
      return {
        content: [{ type: "text", text: result.error ?? result.message }],
        details: result,
        isError: !!result.error,
