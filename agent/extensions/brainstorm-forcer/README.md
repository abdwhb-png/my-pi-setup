...
Allowed non-workflow tool results become bounded, redacted `EV-*` records. Claims are `CL-*`, verification audits are `RV-*`, waivers are `WV-*`, and explicit force overrides are `OV-*`.
