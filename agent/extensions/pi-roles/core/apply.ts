...
export function filterToolsForRuntime(
    ...
    | { kind: "set"; names: string[]; warnings: string[] }
    | { kind: "inherit"; warnings: string[] } {
...
export async function applyRole(
    ...
    if (role.model) {
        ...
        if (!lookup.model) {
            ...
            if (!ok) {
                debugLog(
                    "apply",
                    `setModel refused: ${lookup.model.provider}/${lookup.model.id}`,
                );
                warnings.push(
    ...
    const toolPolicy: RoleToolPolicyPayload =
        filtered.kind === "set"
            ? {
                  version: 1,
                  roleName: role.name,
