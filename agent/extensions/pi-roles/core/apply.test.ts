...
interface FakeApi {
  pi: {
    ...
    events: { emit: ReturnType<typeof mock> };
  };
  ctx: {
    hasUI: boolean;
    ui: { setStatus: ReturnType<typeof mock> };
    modelRegistry: { find: ReturnType<typeof mock>; getAll: ReturnType<typeof mock> };
  };
}
...
function makeFake(
  ...
  const fake: FakeApi = {
    pi: {
      ...
      setActiveTools: mock(),
      getAllTools: mock(() => tools.map((t) => ({ ...t, sourceInfo: {} }))),
      appendEntry: mock(),
      sendMessage: mock(),
      events: { emit: mock() },
    },
...
describe("applyRole", () => {
  ...
  it("publishes the effective role tool policy for cross-extension enforcement", async () => {
    const fake = makeFake({
      tools: [{ name: "@inspect" }, { name: "session_plan" }, { name: "intercom" }],
    });
    const role = makeRole({
      intercom: "both",
      tools: { kind: "set", names: ["@inspect", "session_plan"] },
    });

    await applyRole(role, applyCtxOf(fake));

    expect(fake.pi.events.emit).toHaveBeenCalledWith(ROLE_TOOL_POLICY_EVENT, {
      version: 1,
      roleName: "test",
      mode: "set",
      toolNames: ["@inspect", "session_plan", "intercom"],
    });
  });
