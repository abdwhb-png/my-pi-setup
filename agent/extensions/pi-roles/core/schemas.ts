...
import { type RoleSource } from "../../_shared/pi-roles/contracts.ts";

export {
    ACTIVE_ROLE_ENTRY_TYPE,
    ActiveRoleStateSchema,
    RoleSourceSchema,
    type ActiveRoleState,
    type RoleSource,
} from "../../_shared/pi-roles/contracts.ts";
...
export const IntercomModeSchema = Type.Union(
    ...
    {
        description:
            "Per-role intercom mode. Defaults to global intercomMode setting when omitted.",
    },
);
...
export const RoleFrontmatterSchema = Type.Object(
    {
        ...
        name: Type.String({
            minLength: 1,
            description: "Role identifier; matches the filename.",
        }),
        ...
        description: Type.String({
            minLength: 1,
            description:
                "Short human-readable description of what the role is for.",
        }),
        ...
        model: Type.Optional(
            Type.String({
                description: "Model id; e.g. 'anthropic/claude-opus-4-7'.",
            }),
        ),
        ...
        handoffGuard: Type.Optional(
            Type.String({
                minLength: 1,
                description: "Opaque role-transition policy key.",
            }),
        ),
...
export const PiRolesSettingsSchema = Type.Object(
    {
         ...
         * Default: "pi-agent" (the built-in fallback).
