...
export default function registerPiRolesCore(pi: ExtensionAPI): void {
    ...
    pi.registerFlag(FLAG_NAME, {
        type: "string",
        description:
            "Launch as the named pi-roles role (e.g. --role architect).",
    });
    ...
    pi.registerMessageRenderer<RoleNotificationDetails>(
        ROLE_NOTIFICATION_MESSAGE_TYPE,
        () => {
    ...
    pi.on("session_start", async (event, ctx) => {
        ...
        debugLog(
            "index",
            `session_start reason=${event.reason}`,
            restored ? { name: restored.name } : undefined,
        );
    ...
    pi.on("before_agent_start", async (event, ctx) => {
        ...
        const switchReq = findUnprocessedSwitchRequest(
        ...
        if (switchReq) {
            ...
            const outcome = await applyResolved(
                pi,
                ctx,
                state,
    ...
    pi.registerCommand("role", {
        description:
            "Switch session role. /role list | current | reload | <name> [--reset]",
        getArgumentCompletions: (prefix) =>
            roleCompletions(prefix, state.roles),
...
export function composeSystemPrompt(
    ...
    const intercomMode = effectiveIntercomMode(
        state.activeRole,
        state.settings.intercomMode,
    );
    const addendum =
    ...
    if (!originalPrompt && ctx && ctx.hasUI) {
        ctx.ui.notify(
            "pi-roles: systemPromptMode preserves Pi's original prompt, but no original system prompt was found.",
            "warning",
        );
    }
...
function findRestoredState(
    ...
    for (let i = entries.length - 1; i >= 0; i--) {
        ...
        if (
            e &&
            e.type === "custom" &&
            e.customType === ACTIVE_ROLE_ENTRY_TYPE
        ) {
...
async function applyResolved(
    ...
    try {
        ...
        if (ctx.hasUI) {
            ctx.ui.notify(
                `pi-roles: ${message} Falling back to ${BUILTIN_ROLE_DEFAULT_NAME}.`,
                "warning",
            );
        }
        const fallback = state.roles.find(
            (r) =>
                r.frontmatter.name === BUILTIN_ROLE_DEFAULT_NAME &&
                r.source === "built-in",
        );
        if (!fallback) {
    ...
    refreshRoleWidget(ctx, state.activeRole, state.settings.showWidget ?? true);

    debugLog("index", `applied role=${resolved.name}`, {
    ...
    if (ctx.hasUI && result.warnings.length > 0 && !options.silent) {
        ...
        for (const w of result.warnings)
            ctx.ui.notify(`pi-roles: ${w}`, "warning");
    }
...
async function handleList(
    ...
    const lines = state.roles
        ...
        .map((r) => {
            const marker =
                state.activeRole?.name === r.frontmatter.name ? "* " : "  ";
    ...
    const all =
        shadowed.length > 0
            ? [
                  ...
                  ...lines,
                  "",
                  "Shadowed (lower-priority duplicates):",
                  ...shadowed,
              ]
...
async function handleCurrent(
    ...
    const chain =
        r.extendsChain.length > 1
            ? ` (extends: ${r.extendsChain.slice(1).join(" → ")})`
            : "";
    ctx.ui.notify(
        `pi-roles: ${r.name}${chain} — ${r.description}\n${r.path}`,
        "info",
    );
}
