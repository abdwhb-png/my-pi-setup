...
export function discoverRoles(cwd: string, scope: RoleScope): DiscoveryResult {
    ...
    if (scope === "project" || scope === "both") {
        ...
        buckets.push({
            source: "project",
            files: projectDir ? listRoleFiles(projectDir) : [],
        });
    }
    ...
    buckets.push({
        source: "built-in",
        files: listRoleFiles(builtInRolesDir()),
    });
...
export function parseRoleSource(
    ...
    try {
        ...
        throw new RoleResolutionError(
            `${path}: invalid YAML in frontmatter — ${detail}`,
        );
    }
    ...
    if (!Value.Check(RoleFrontmatterSchema, parsed)) {
        ...
        throw new RoleResolutionError(
            `${path}: invalid frontmatter at ${where} — ${why}`,
        );
    }
...
export function resolveRole(name: string, all: RawRole[]): ResolvedRole {
    ...
    while (cursor) {
        if (seen.has(cursor.frontmatter.name)) {
            const cyclePath = [
                ...chain.map((r) => r.frontmatter.name),
                cursor.frontmatter.name,
            ];
            throw new RoleResolutionError(
        ...
        if (!parent) {
            debugLog(
                "roles",
                `extends not found: ${cursor.frontmatter.name} -> ${parentName}`,
            );
            throw new RoleResolutionError(
...
export function findBuiltInAssistant(roles: RawRole[]): RawRole | undefined {
    return roles.find(
        (r) =>
            r.frontmatter.name === BUILTIN_ROLE_ASSISTANT_NAME &&
            r.source === "built-in",
    );
}
