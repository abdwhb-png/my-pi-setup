...
function toLocalEditPath(path: string, remoteCwd: string): string {
    if (path.startsWith("~/")) {
        throw new Error(
            "ssh_edit does not expand ~ paths. Use a path relative to the SSH working directory instead.",
        );
    }
    if (isAbsolute(path)) {
...
function toRemotePath(
    ...
    if (relativePath.startsWith("../") || relativePath === "..") {
        throw new Error(
            `Resolved edit path ${path} escaped the local SSH edit workspace.`,
        );
    }
    ...
    return new Promise<{
        ...
        const stdinPrefix =
            options.stdin === undefined
                ? ""
                : `printf %s ${shellQuote(Buffer.from(options.stdin).toString("base64"))} | base64 -d | `;
        const child = spawn("ssh", [remote, `${stdinPrefix}${command}`], {
...
function createRemoteReadOps(target: ActiveSshTarget): ReadOperations {
    return {
        readFile: (absolutePath) =>
            sshOk(target.remote, `cat ${shellQuote(absolutePath)}`),
        access: (absolutePath) =>
            sshOk(target.remote, `test -r ${shellQuote(absolutePath)}`).then(
                () => {},
            ),
        detectImageMimeType: async (absolutePath) =>
            inferImageMimeType(absolutePath),
    };
}
...
function createRemoteWriteOps(target: ActiveSshTarget): WriteOperations {
    return {
        ...
        mkdir: (dir) =>
            sshOk(target.remote, `mkdir -p ${shellQuote(dir)}`).then(() => {}),
    };
}
...
function createRemoteEditOps(
    ...
    const remotePath = (path: string) =>
        toRemotePath(path, localCwd, target.remoteCwd);
    return {
        readFile: (absolutePath) =>
            sshOk(target.remote, `cat ${shellQuote(remotePath(absolutePath))}`),
        writeFile: async (absolutePath, content) => {
            await sshOk(
                target.remote,
                `cat > ${shellQuote(remotePath(absolutePath))}`,
                { stdin: content },
            );
        },
        access: (absolutePath) => {
...
export default function sshToolsExtension(pi: ExtensionAPI) {
    ...
    pi.registerTool({
        ...
        async execute(toolCallId, params, signal, onUpdate, ctx) {
            ...
            const tool = createEditToolDefinition(localCwd, {
                operations: createRemoteEditOps(target, localCwd),
            });
            return tool.execute(
                toolCallId,
                transformedParams,
    ...
    pi.registerTool({
        ...
        label: "ssh_bash",
        description:
            "Execute a bash command on the active SSH host in the active remote working directory.",
    ...
    pi.registerCommand("ssh", {
        ...
        getArgumentCompletions: (prefix) => {
            const options = [
                "off",
                "status",
                ...refreshProfiles().map((profile) => profile.name),
            ];
