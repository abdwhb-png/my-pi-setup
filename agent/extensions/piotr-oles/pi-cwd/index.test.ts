...
import piCwd from './index.ts';
import type { CwdMode } from './mode.ts';

function cwdExtension(mode: CwdMode) {
    return (pi: any) => {
        const orig = pi.getFlag.bind(pi);
        pi.getFlag = (name: string) =>
            name === 'pi-cwd-mode' ? mode : orig(name);
        piCwd(pi);
    };
}
    ...
    describe('warn mode (default)', () => {
        ...
        describe('bash tool', () => {
            it('appends tip when command contains absolute path', async () => {
                ...
                await t.run(
                    when('Run bash', [
                        calls('bash', { command: `cat ${t.cwd}/file.ts` }),
                        says('Done.'),
                    ]),
                );
            ...
            it('no tip when command uses only relative paths', async () => {
                ...
                await t.run(
                    when('Run bash', [
                        calls('bash', { command: 'cat src/file.ts' }),
                        says('Done.'),
                    ]),
                );
