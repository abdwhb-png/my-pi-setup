...
const PROMPT = `Run git status, inspect what has changed, then respond with only:
...
export default function (pi: ExtensionAPI) {
	pi.registerCommand("summarize-unstaged", {
		description: "Summarize unstaged git changes with per-file +/- counts",
		handler: async (_args, ctx) => {
			if (!ctx.isIdle()) {
				pi.sendUserMessage(PROMPT, { deliverAs: "followUp" });
				ctx.ui.notify("Queued /summarize-unstaged after the current turn finishes.", "info");
				return;
			}
