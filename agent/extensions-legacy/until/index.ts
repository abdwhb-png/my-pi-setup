...
import { createWidget } from "../../extensions/_shared/fancy-footer.ts";
import { createUiColors } from "../../extensions/_shared/ui/ui-colors.ts";
...
async function selectSummaryModel(
    ...
    if (ctx.model.provider === "anthropic") {
        ...
        if (haikuModel) {
            const resolved =
                await ctx.modelRegistry.getApiKeyAndHeaders(haikuModel);
            if (resolved.ok && resolved.apiKey) {
                return { model: haikuModel, apiKey: resolved.apiKey };
            }
        }
    }

    const resolved = await ctx.modelRegistry.getApiKeyAndHeaders(ctx.model);
    if (!resolved.ok || !resolved.apiKey) return null;
    return { model: ctx.model, apiKey: resolved.apiKey };
}
...
export default function loopExtension(pi: ExtensionAPI): void {
    ...
    async function showLoopSelector(
        ...
        const selection = await ctx.ui.custom<string | null>(
            (tui, theme, _kb, done) => {
                ...
                container.addChild(
                    new DynamicBorder((str) => colors.primary(str)),
                );
                container.addChild(
                    new Text(
                        colors.primary(theme.bold("Select a loop preset")),
                    ),
                );
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
            if (!loopState.active) {
                return {
                    content: [
                        { type: "text", text: "No active loop is running." },
                    ],
    ...
    pi.registerCommand("until", {
        description:
            "Repeat until a condition is met (e.g., /until tests, /until custom <condition>)",
    ...
    pi.on("session_before_compact", async (event, ctx) => {
        ...
        const apiKey = await ctx.modelRegistry.getApiKeyAndHeaders(ctx.model);
        if (!apiKey.ok || !apiKey.apiKey) return;
