...
function getArgumentCompletions(prefix: string) {
    ...
    const filtered = options.filter((option) =>
        option.value.startsWith(trimmed),
    );
...
export default function modelThinkingExtension(pi: ExtensionAPI) {
    ...
    pi.on("thinking_level_select", async (event, ctx) => {
        ...
        if (
            lastProgrammaticSet &&
            lastProgrammaticSet.modelKey === currentKey &&
            lastProgrammaticSet.level === event.level
        ) {
