...
If `pi-expert` is unavailable or fails to provide usable guidance, delegate to another suitable subagent such as `Codebase Scout` or `Librarian`. If no suitable subagent is available or delegation fails, self-exploration is allowed. In every case, verify assumptions against the local harness code, `docs/ABOUT-PI.md`, official documentation. Never guess `pi` APIs, types, or architecture.
