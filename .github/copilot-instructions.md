...
**MANDATORY DELEGATION:** When planning, exploring, or implementing changes to the `pi` agent harness (anything within `~/.pi/` or `~/.pi/agent/` or related core logic), you are **STRICTLY FORBIDDEN** from performing solo exploration. You MUST delegate the discovery and architectural verification phase to the `pi-expert` agent using `runSubagent` tool.
